...
| :--- | :--- | :--- | :--- |
| **`FOR_GRID_ONLY`** | **Сеточник** (`cron3Papper`) | Не используется (автономно) | Исключительно метрики сетки: $\text{N/R} \ge 0.70$, $\text{R/R} < 1.0$, $\text{MDME} < 0.05$, $\text{Grid Net} > 0$. Чистый флэт. |
...
1. **`FOR_GRID_ONLY` (Pure Grid Standalone)**:
   ...
   * Критерии: Отбираются чистые «Денежные коровы» с максимальным `N/R`, минимальной просадкой `R/R` и обязательным положительным `Grid Net`. Данные TrendH не учитываются.
