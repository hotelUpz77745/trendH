...
STRATEGY_DESCRIPTIONS: Dict[str, Dict[str, str]] = {
    ...
    "u_shadow_harvester_40": {
        "name": "Shadow Harvester 40% (Grid Reverse + Trailing) 🌾",
        "concept": "<b>Оцифровка плавающей прибыли теневого хеджа.</b> Входит против застрявшей сетки 40% (Lvl 2+) и динамически фиксирует прибыль трейлингом вместо пассивного ожидания закрытия сетки.",
        "entry": "• <b>Grid Stress (Level 2+)</b>: Сетка застряла (ratio >= 0.40, filled >= 2).\n• <b>Direction</b>: Против зависшей стороны сетки.\n• <b>Graduated Sizing</b>: 75% L2, 55% L3, 40% L4.",
        "exit": "• <b>Breakeven Ratchet</b>: Безубыток (+0.5%) при +2.5% + трейлинг 1.8% от пика.\n• <b>Grid Relief</b>: Выход при закрытии сетки по TP.\n• <b>Stop Loss</b>: -3.8%.",
        "note": "🌾 <b>Сборщик урожая сетки</b>: Превращает висящие нереализованные иксы в зафиксированный кэш."
    },
    "u_shadow_harvester_50": {
        "name": "Shadow Harvester 50% (Grid Reverse + Trailing) 🌾",
        "concept": "<b>Умеренный сборщик урожая сетки.</b> Входит при застревании 50% объема сетки (Lvl 2+) с динамическим трейлингом прибыли.",
        "entry": "• <b>Grid Stress (Level 2+)</b>: Сетка застряла (ratio >= 0.50, filled >= 2).\n• <b>Direction</b>: Строго против зависшей стороны.",
        "exit": "• <b>Breakeven Ratchet</b>: Трейлинг 1.8% от пика при +2.5%.\n• <b>Grid Relief</b>: Выход по TP сетки.\n• <b>Stop Loss</b>: -3.8%.",
        "note": "🌾 <b>Сборщик урожая 50%</b>: Более консервативный фильтр глубины застревания."
    },
    "u_shadow_harvester_mid": {
        "name": "Shadow Harvester Mid (4.5% / 2.5% Trail) 🌾",
        "concept": "<b>Свинг-Жнец 50%.</b> Промежуточный сборщик урожая: отсекает 5m шум и не режет прибыль на микро-откатах, активируя трейлинг только после +4.5%.",
        "entry": "• <b>Grid Stress (Level 2+)</b>: Сетка застряла (ratio >= 0.50, filled >= 2).\n• <b>Direction</b>: Против зависшей сетки.",
        "exit": "• <b>Breakeven Ratchet</b>: Активация при +4.5%, трейл 2.5% от пика, БУ +1.5%.\n• <b>Grid Relief</b>: Выход по TP сетки | <b>Stop Loss</b>: -4.0%.",
        "note": "🌾 <b>Свинг-Жнец</b>: Экономит комиссии и высиживает импульсы от 5% до 10%."
    },
    "u_shadow_harvester_wide": {
        "name": "Shadow Harvester Wide (7.0% / 3.5% Trail) 🌾",
        "concept": "<b>Трендовый Жнец 50%.</b> Максимально близок к чистому хэджу: сидит до победного вместе с сеткой, но защищает супер-профиты (+10..+40%) трейлингом 3.5% от пика.",
        "entry": "• <b>Grid Stress (Level 2+)</b>: Сетка застряла (ratio >= 0.50, filled >= 2).\n• <b>Direction</b>: Против зависшей сетки.",
        "exit": "• <b>Breakeven Ratchet</b>: Активация при +7.0%, трейл 3.5% от пика, БУ +3.0%.\n• <b>Grid Relief</b>: Выход по TP сетки | <b>Stop Loss</b>: -5.0%.",
        "note": "🌾 <b>Трендовый Жнец</b>: Забирает гигантские выносы и не дает профиту растаять при развороте."
    },
    "u_grid_shadow_be": {
        "name": "Pure Grid Shadow (Breakeven Lock) 🛡️",
        "concept": "<b>Сеточный хэдж 50% с БУ-замком.</b> Позволяет позиции расти без помех от трейлинга, но при достижении +2.8% жестко фиксирует стоп в безубыток (+0.3%).",
        "entry": "• <b>Grid Stress (Level 2+)</b>: Сетка застряла (ratio >= 0.50, filled >= 2).\n• <b>Direction</b>: Против зависшей сетки.",
        "exit": "• <b>Breakeven Lock</b>: Фиксация БУ (+0.3%) при +2.8% (без трейлинга).\n• <b>Grid Relief</b>: Выход по TP сетки | <b>Stop Loss</b>: -5.0%.",
        "note": "🛡️ <b>Замок безубытка</b>: Защищает от просадки -$200, не сбивая сделку на микро-откатах."
    },
    "u_grid_shadow_stagnation": {
        "name": "Pure Grid Shadow (Profit Stagnation) ⏳",
        "concept": "<b>Сеточный хэдж с датчиком стагнации.</b> При профите >5% и отсутствии нового хая в течение 30 минут закрывает сделку с рынка на вершине.",
        "entry": "• <b>Grid Stress (Level 2+)</b>: Сетка застряла (ratio >= 0.50, filled >= 2).\n• <b>Direction</b>: Против зависшей сетки.",
        "exit": "• <b>Profit Stagnation</b>: Закрытие при затухании импульса (>5% и 30 мин без хая).\n• <b>BE Lock</b>: БУ (+0.3%) при +2.8% | <b>Grid Relief</b>: Выход по TP сетки.",
        "note": "⏳ <b>Оцифровщик пиков</b>: Не ждет часами разгрузки сетки, забирая прибыль на вершине."
    },
    "u_symbiosis_harvester": {
        "name": "Symbiosis Harvester 50% (1H + 15m HVH + Grid) 🧬",
        "concept": "<b>Симбиоз снайпера (WR 80%) и Жнеца 50%.</b> Вход на откате 15m HVH по тренду 1H при стрессе сетки со свинг-трейлингом.",
        "entry": "• <b>1H Trend + 15m HVH Pullback</b>: Вход по тренду на перерастяжке волатильности.\n• <b>Grid Stress (Level 2+)</b>: Сетка 50%.",
        "exit": "• <b>Swing Trailing</b>: Трейлинг 2.5% от пика при +4.5%, БУ +1.5%.\n• <b>Grid Relief</b>: Выход по TP сетки | <b>Stop Loss</b>: -4.0%.",
        "note": "🧬 <b>Абсолютный снайпер-жнец</b>: Максимальная точность входа + свинг-фиксация."
    },
    "u_delta_sniper_15m": {
        "name": "Delta Sniper 15m (Harvester + 15m HVH Impulse) 🎯",
        "concept": "<b>Старший снайпер институциональных импульсов.</b> Входит против сетки при подтверждении импульсного пробоя HVH на таймфрейме 15m.",
        "entry": "• <b>Grid Stress (Level 2+)</b>: ratio >= 0.40, filled >= 2.\n• <b>HVH Impulse (15m)</b>: Пробой полосы волатильности 15m (dev=1.6).\n• <b>Momentum Guard</b>: Свеча 5m сонаправлена + RSI >= 48.",
        "exit": "• <b>Breakeven Ratchet</b>: Трейлинг 1.8% от пика при +2.5%.\n• <b>Grid Relief</b>: Выход по TP сетки | <b>Stop Loss</b>: -3.8%.",
        "note": "🎯 <b>Снайпер 15m</b>: Фильтрует 5m шум и бьет по крупным институциональным импульсам."
    },
    "u_hvh_delta_symbiosis": {
        "name": "HVH Delta Symbiosis (1H Trend + 15m Pullback + Trailing) 🧬",
        "concept": "<b>Симбиоз лидеров №1 и №2.</b> Вход на откате к полосе 15m HVH по тренду 1H при стрессе сетки (Lvl 1+) с последующим динамическим трейлингом.",
        "entry": "• <b>Trend H1</b>: Старший тренд (UP/DOWN).\n• <b>HVH Pullback (15m)</b>: Откат к границе волатильности.\n• <b>Grid Stress (Level 1+)</b>: Застревание сетки (ratio >= 0.25).",
        "exit": "• <b>Breakeven Ratchet</b>: Трейлинг 1.8% от пика при +2.5%.\n• <b>Grid Relief</b>: Выход по TP сетки | <b>Stop Loss</b>: -3.8%.",
        "note": "🧬 <b>Абсолютный симбиоз</b>: Идеальная точка входа со скидкой + фиксация верхушки трейлингом."
    },
    "u1": {
