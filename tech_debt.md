...
2. **Динамический сайзинг работает по формуле `accum_usd * 0.5`:** Объем берется не просто от базового ордера (`0`), а от суммы всех фактически исполненных уровней сетки, помноженной на коэффициент хеджирования 0.5 (50%).
